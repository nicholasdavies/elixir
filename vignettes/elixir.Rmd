...
Frequent users of the excellent [`stringr`](https://stringr.tidyverse.org/)
package will recognize the intentional similarity between the above functions
and `str_match()`, `str_replace()`, `str_count()`, `str_detect()`,
`str_extract()`, and `str_locate()` from `stringr`.
